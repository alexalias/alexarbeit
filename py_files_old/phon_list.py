		...
		for line in work_file:
			...
			if re.match("MAU", line):
				if str(line.split()[4]) in valid_phonemes:
					...
					# Here we fill the list with information from each file
					# Phoneme id
					c_list.append(p_key)

					# Filename just for the record
					c_list.append(str(datei)[-22:])
					...
					if p_key in vowel_list:
						...
						c_list.append([ key[0] for key, value in ctype_dict.items() if p_key in value][0])
					...
					c_list.append(get_syl_part(datei, p_key, int(line.split()[3]), int(line.split()[6])))
					...
					if nucleus_ind != "NaN":
						c_list.append(syl_list[nucleus_ind+1])
					...
					if len(memo_list) > 0:
						if memo_list[1] in vowel_list:
							...
							c_list.append([ key[0] for key, value in ctype_dict.items() if memo_list[1] in value][0])
					else:
						c_list.append("none")
						c_list.append("none")
					...
					if len(following_phoneme) > 0:
						if following_phoneme[1] in vowel_list:
							...
							c_list.append([ key[0] for key, value in ctype_dict.items() if following_phoneme[1] in value][0])
					...
					if len(following_phoneme) > 0:
						c_list.append(get_syl_part(datei, following_phoneme[1], following_phoneme[0], following_phoneme[2]))
					else:
						c_list.append("none")

					# Speech rate calculated as word_duration / phoneme_count_in_word
					c_list.append(new_SR.get_word_SR(datei, int(line.split()[3]))[0])

					# Duration of current phoneme in msec
					c_list.append(round(int(line.split()[2]) * 0.0625, 2))
